...
char *cmd[] = { "getpid", "ps", "chname", "kmode", "switch", "wait", "exit", "kgetc", "kputc", "fork", "exec", "pipe", "pread", "pwrite", "pclose", "pfd", "itimer", "sin", "sout", 0 };
...
int sin()
{
  ...
  // Clear buffer so we don't accidentally use old data
  for (i = 0; i < 128; i++)
  {
    buffer[i] = 0;
  }
